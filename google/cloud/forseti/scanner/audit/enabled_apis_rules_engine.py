...
def _check_whitelist_apis(rule_apis=None, enabled_apis=None):
    """Whitelist: Check that enabled APIs ARE in rule APIs.
...
def _check_blacklist_apis(rule_apis=None, enabled_apis=None):
    """Blacklist: Check that enabled APIs ARE NOT in rule APIs.
...
def _check_required_apis(rule_apis=None, enabled_apis=None):
    """Required: Check that rule APIs are in enabled APIs.

    If a required rule API is NOT found enabled in the project, add it to the
    violating APIs. Note that the check is different:
    it's reversed from the whitelist/blacklist (enabled as a subset of
    rules vs rules as subset of enabled).
    ...
    Returns:
        list: Rule APIs not found enabled by the project (required-whitelist).
    """
